//! クエリランタイム (購読の台帳 / 差分バッファ / 読み取りモデル)。Tauri を知らない (#1106)。
//! IPC コマンド、delta flusher、イベント名の宣言は commands/query.rs。
...
pub(crate) const WARM_GRACE: Duration = Duration::from_millis(8000);
/// 高頻度 stream event を 1 フレーム分まとめて 1 個の query-delta event に
/// 集約するための debounce 窓。連続イベント時に IPC 数を桁で減らせる。
pub(crate) const DELTA_FLUSH_WINDOW: Duration = Duration::from_millis(16);
