...
notecli は NoteDeck の Rust バックエンドから Tauri 非依存のコードを分離したクレート。
当初は独立リポジトリだったが、2026-09-23 に `crates/notecli` として取り込んだ (#1106)。
...
- [x] **ハッシュタグトレンド** — AiScript ウィジェット (`trending-hashtags`) として実装済み。
...
- [x] **アカウント別ウィジェット管理** — ウィジェットカラムを per-account / 全アカウントで
  開ける。「インストール済み」「ストア」の 2 タブ構成で、ライブラリ (= storeId 有無で
  ストア由来/ローカル) と「カラムへの配置」を二層モデルに分離。
  ヘッダー＋ボタンで新規ローカルウィジェットを作成し、widget-edit window
  (コード/ビジュアル 2 タブ) で編集。ライブラリピッカーから既存ウィジェットを配置・編集・削除。
