...
- **開発者モード (#1034)**: 開発者向けの面 (API コンソール / ストリーム / スクラッチパッド / Raw JSON / タスク / スキル / AI) は既定で隠し、トグルで開放する。面には帰属タグ `exposure` を持たせ、判定は `src/settings/exposure.ts` の `isExposed()` 一本。**隠すのは入口だけ**でデッキ上のカラムは動き続け、認可の境界でもない。新しい面を足すときは「既定で見せるか」を決めてタグを付ける (詳細は [DEVELOPMENT.md](DEVELOPMENT.md) の "開発者モードと露出タグ")
