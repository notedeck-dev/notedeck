...
- **リモート構成での「手元」** — notecore を自分のサーバー (notenode) で常駐させる構成 ([#1106](https://github.com/notedeck-dev/notedeck/issues/1106)) では、これらの原則の「手元」は端末ではなく自分が運用するホストを指す。ファイルが source of truth であることは変わらず、設定はサーバー側の束と端末側の束 (入力・画面・端末性能に依存するもの) に分かれる
