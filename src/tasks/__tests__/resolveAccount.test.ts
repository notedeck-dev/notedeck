...
describe('resolveTaskAccount', () => {
  it('明示 id があれば一致するアカウントを返す', () => {
    const r = resolveTaskAccount([a1, a2], 'a2')
    expect(r).toEqual({ ok: true, id: 'a2', host: 'misskey.cloud' })
  })

  it('明示 id が見つからなければ not-found (フォールバックしない)', () => {
    const r = resolveTaskAccount([a1], 'gone')
    expect(r).toEqual({ ok: false, reason: 'not-found', requestedId: 'gone' })
  })

  it('明示 id なしは最初のトークン保持アカウントへフォールバックする (#941)', () => {
    expect(resolveTaskAccount([a1, a2], undefined)).toEqual({
      ok: true,
      id: 'a1',
      host: 'yami.ski',
    })
    expect(resolveTaskAccount([guest, a1], null)).toEqual({
      ok: true,
      id: 'a1',
      host: 'yami.ski',
    })
  ...
  it('利用可能なアカウントが無ければ no-account', () => {
    const r = resolveTaskAccount([guest], undefined)
    // guest は hasToken=false なので fallback 探索では対象外
