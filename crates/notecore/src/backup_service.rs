//! バックアップ作成 (#816)
//!
//! 保存先 (ダウンロードフォルダ) の解決は手元側 (src-tauri の commands/backup.rs)、
//! 本体はここ (#1106 段階 0b、旧 mixed の分割)。
//!
...
//! - デッキプロファイル・プラグイン登録・テーマ選択の正本は設定フォルダのファイル (#913) なので
//!   設定バンドルに含まれる。localStorage は起動時の即時復元用のミラーで対象外
//! - 世代は新しい順に `keep` 件だけ残す
...
pub async fn create(
    ...
    let settings_files = if include_settings {
        let settings_base = core.app_dir()?.join("notedeck");
