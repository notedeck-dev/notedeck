...
pub const VALID_AI_PROVIDERS: &[&str] = &["anthropic", "openai", "custom"];

pub fn validate_ai_provider(provider: &str) -> Result<()> {
...
pub fn ai_keychain_id(provider: &str) -> String {
    format!("ai.{provider}")
}
...
pub fn read_ai_api_key(provider: &str) -> Result<Option<String>> {
