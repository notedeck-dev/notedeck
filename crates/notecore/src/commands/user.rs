//! user のデータ系コマンド本体 (#1106 段階 0b)。各関数は `&Core` と引数を取り、
...
pub async fn api_get_self(core: &Core, account_id: String) -> Result<serde_json::Value> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.get_self(&host, &token).await
}

pub async fn api_follow_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.follow_user(&host, &token, &user_id).await
}

pub async fn api_unfollow_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
...
pub async fn api_mute_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.mute_user(&host, &token, &user_id).await
}

pub async fn api_unmute_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.unmute_user(&host, &token, &user_id).await
}

pub async fn api_renote_mute_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
...
pub async fn api_get_muted_users(core: &Core, account_id: String) -> Result<Vec<String>> {
    let (client, host, token) = core.authed(&account_id).await?;
...
pub async fn api_get_muted_words(core: &Core, account_id: String) -> Result<MutedWordsResult> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.muted_words(&host, &token).await
}

/// 自分が renote mute 中のユーザー ID 一覧を取得する（#614: 起動時の renote mute store hydrate）。
pub async fn api_get_renote_muted_users(core: &Core, account_id: String) -> Result<Vec<String>> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.renote_muted_user_ids(&host, &token).await
}

pub async fn api_block_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
    client.block_user(&host, &token, &user_id).await
}

pub async fn api_unblock_user(core: &Core, account_id: String, user_id: String) -> Result<()> {
    let (client, host, token) = core.authed(&account_id).await?;
