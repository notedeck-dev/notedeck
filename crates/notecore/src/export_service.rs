...
//!
//! 手元側 (src-tauri の commands/export.rs) は保存先 (ダウンロードフォルダ) の解決と
//! Tauri イベントへの進捗転送だけを持ち、本体はここ (#1106 段階 0b、旧 mixed の分割)。
...
use crate::ssrf::validate_external_url;
use crate::ssrf::PinningResolver;
...
pub fn cancel(task_id: &str) {
    CANCELLED.lock().unwrap().insert(task_id.to_string());
}

/// エクスポートを開始し、解決済みの保存先ディレクトリを返す。
/// 即座に返り、以後の進捗は `progress` に流れる。
pub fn start(
    root: PathBuf,
    task_id: String,
    segments: Vec<String>,
    ...
    tokio::spawn(run_export(progress, task_id, dir, items));
    Ok(dir_str)
}
...
async fn run_export(
    ...
    let client = match build_client() {
        Ok(c) => c,
        Err(e) => {
            emit(&progress, &task_id, "", "failed", Some(e), 0, total);
            emit(&progress, &task_id, "", "finished", None, 0, total);
            return;
        }
    };
            ...
            async move {
                ...
                match result {
                    Ok(()) => {
                        ...
                        emit(&progress, &task_id, &item.file_id, "done", None, d, total);
                    }
                    Err(e) => emit(
                        &progress,
                        &task_id,
