...
| 生ファイルを直接編集するタブ（アピアランス / キーバインド / テーマ / 権限）と、その「OS 既定エディタで開く」 | 各ウィンドウが宣言的に opt-in |
...
**トグル:** コマンドパレットのトグルコマンドと、「NoteDeck について」のスイッチ。設定メニューには置かない。このほか、AI プロトコル付きの接続を保存したときに一度だけ有効化を尋ねる (登録した鍵が何にも使えない袋小路を作らないため)。読み書きは `useDeveloperMode` に集約している。

**off でも動き続けるもの:** HEARTBEAT は開発者モードと無関係に走る。off にすると制御面の入口が消えるので、切り替え時に知らせ、「NoteDeck について」に稼働バッジと停止ボタンを常設している。
