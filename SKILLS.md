...
builtin は **144 個 / 39 subject** (v0.24.0 時点、`src/capabilities/builtins/` 配下の unique id)。subject 別にグループ化:
...
| **テーマ** | `theme.list`, `theme.read`, `theme.apply`, `theme.create`, `theme.update`, `theme.history`, `theme.revert` | per-account テーマ CRUD + 編集履歴 |
