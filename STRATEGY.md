...
3. **環境の書き換え** — テーマ・ウィジェット・プラグイン・スキルを自分で書き、自分で適用する
...
同じ到達点をユーザー視点で案内するのは [AI と環境を育てる](site/docs/guide/grow.md)。技術構成には触れず、段階を追って使い込むガイドとして書く。この節を書き換えたら、あちらも合わせて直す。
...
| **履歴が問い合わせ可能である** | 過去の投稿・反応を、その場の思いつきの条件で引ける |
| **定期実行が拡張されたものを呼べる** | HEARTBEAT が、組み込みの機能だけでなく AI 自身が作ったコマンドも実行できる |
| **外部の情報が環に入る** | 接続した外部サービスから取った情報が、拡張を作る材料になる |

観測・判断・環境の書き換え・検証にあたる部品はすでに動いている。AI はテーマを作って適用でき、スキルを書き換えて自分の人格を更新でき、AiScript を書いて診断を見ながら直し、Misskey の操作を代行し、定期的に自分で起きる。**足りていないのは部品ではなく、上の表の接続である。** どこまで繋がったかは [ROADMAP.md](ROADMAP.md) の Layer 4 で追う。
