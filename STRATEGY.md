...
この表はローカル構成 (アプリに notecore を埋め込む) の話。リモート構成 ([#1106](https://github.com/notedeck-dev/notedeck/issues/1106)) では全 Misskey トークンと AI の API キーが自分のサーバーのファイルに置かれ (データディレクトリの外の鍵で暗号化)、「鍵は OS キーチェーンから出ない」の訴求は成り立たない。代わりに得るのは「鍵は自分のサーバーから出ない」で、その安全性は利用者の運用に依存する。
