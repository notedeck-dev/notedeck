...
notecli は notedeck のコア基盤となる Rust クレートであり、**スタンドアロン CLI** と **ライブラリ** の二重の役割を持つ。目指す構成 (#1106) でも役割は変わらず、notecore がその消費者になる。
...
| 6 | WebView 用 `createQuerySubscription`（旧 `useQuerySubscription` は削除済み） | **稼働中** | `c8473902` / `351a34b4` |
